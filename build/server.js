...
server.registerTool("save-memory", {
    title: "Memory Agent",
...
server.registerTool("query-memory", {
    ...
    try {
        const topK = 3;
        const queryEmbedding = await generateEmbeddings(text);
        const memories = await Memory.find();
...
server.registerTool("delete-memory", {
...
server.registerTool("episodic-memory", {
...
server.registerTool("query-episodic", {
...
server.registerTool("save-relationship", {
...
server.registerTool("relationship-query", {
...
server.registerTool("save-goalorhabbit", {
    ...
    try {
        ...
        const isHabbitOrGoal = await detectHabitOrGoal(text);
        var habbitFrequency = await extractHabitFrequency(text);
        var goalDeadline = await extractGoalDeadline(text);
        ;
        if (bestScore > threshold) {
            bestMatch.text = text;
            bestMatch.embeddings = embedding;
            ...
            await bestMatch.save();
            if (isHabbitOrGoal === "habit")
                await tryLinkHabitToGoal(bestMatch);
        }
        else {
            ...
            await memory.save();
            if (isHabbit)
                await tryLinkHabitToGoal(memory);
        }
        return ({
...
server.registerTool("update-goalProgress", {
...
server.registerTool("habit-checkin", {
...
async function checkIfTextExist(text, embeddings) {
    const memories = await Memory.find();
    var bestScore = 0;
    var bestMatch = null;
    if (memories) {
